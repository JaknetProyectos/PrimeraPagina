...
export default function AdventurePlannerSection() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            // 1. Guardar en Supabase a través del hook
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative bg-white text-black w-full max-w-md overflow-hidden rounded-sm shadow-2xl animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="p-10 text-center">
                    ...
                    <p className="text-xs text-gray-500 mb-8 font-medium px-4">Tu folio ha sido generado. Recibirás una propuesta en menos de 24 horas.</p>
                    <div className="mb-8 text-left">
                        <label className="text-[10px] font-bold text-[#03A9F4] uppercase block mb-1 ml-1">ID de Seguimiento</label>
                        <div className="flex items-center gap-3 bg-gray-50 border-b-2 border-gray-200 p-4 group cursor-pointer hover:bg-gray-100 transition-all" onClick={() => { navigator.clipboard.writeText(quoteId); alert("Copiado"); }}>
                            <code className="flex-grow font-mono font-bold text-sm text-[#212121]">{quoteId}</code>
                            <Copy size={16} className="text-gray-400 group-hover:text-[#03A9F4]" />
                        </div>
                    </div>
                    <div className="space-y-3">
                        ...
                        <Link href={`/pagatuaventura?quoteId=${quoteId}`} className="w-full inline-block border-2 border-gray-200 text-gray-600 py-3 rounded-sm font-bold uppercase text-xs tracking-widest hover:bg-gray-50 text-center">Ver Estatus en vivo</Link>
                    </div>
                </div>
            </div>
        </div>
